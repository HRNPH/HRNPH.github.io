 ...
 * @returns An array of objects indicating whether the segment is a regex match and the corresponding text.
*/
function split(text: string, regex: RegExp): { match: boolean; text: string }[] {
    const result: { match: boolean; text: string }[] = [];
    let prev = 0;
    for (const match of text.matchAll(regex)) {
        // Using non-null assertion since match.index is defined for matches.
 ...
 * @param match The matched currency string.
 ...
 * @param language The language to use ("a" for American English or "b" for British English).
 * @param norm Whether to normalize the text.
 * @returns A Promise that resolves to the phonemized text.
 */
export async function phonemize(
    ...
    // 2. Split into chunks to preserve punctuation
    const sections: { match: boolean; text: string }[] = split(
        text,
        PUNCTUATION_PATTERN
    );
    ...
    const ps = (
        await Promise.all(
            sections.map(async ({ match, text }): Promise<string> => {
                return match
                    ? text
                    : (await espeakng(text, lang) as string[]).join(" ");
            })
        )
    ).join("");
